(function(global, factory) {
  ...
  function init(args, success, failure) {
    ...
    var hParams = getRequestParameters(
    ...
    state.apiServer =
      args['api_server'] || params['api_server'] || OK_API_SERVER;
    state.widgetServer =
      args['widget_server'] || params['widget_server'] || OK_CONNECT_URL;
  ...
  function wrapCallback(success, failure, dataProcessor) {
    return function(status, data, error) {
      if (status == 'ok') {
        if (isFunc(success))
          success(isFunc(dataProcessor) ? dataProcessor(data) : data);
      } else {
  ...
  function widgetMediatopicPost(returnUrl, options) {
    ...
    options.attachment = btoa(
      unescape(encodeURIComponent(toString(options.attachment))),
    );
